...
`search` expects the `memory-manifest.json` generated by `inspect`. It rejects
documents without a `messages` array and message records without valid
`citation`, `role`, or `content` fields, reporting the exact invalid field.
